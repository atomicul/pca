...
items::CompositeItem *Market::registerCompositeItem(std::string name,
    ...
    if(getItem(name) != nullptr)
        throw std::runtime_error("UNIQUE_NAME_VIOLATION: Item names should be unique.");
    ...
    if (rng::any_of(asIngredients, [](auto const &ingredient) { return ingredient.item == nullptr; }))
        throw std::runtime_error("INVALID_INGREDIENT: All ingredients should be registered before creating a composite item.");
